...
function requireCommands(area, operation) {
    ...
    const glob = path.posix.resolve(__dirname, '..', '..', 'lib', area, operation, '*Command.js');
    logger.debug(area.blue, operation.cyan, 'glob:'.gray, glob);

    const commandFiles = globby.sync(glob);
