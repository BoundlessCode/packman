...
async function updateDistTagToLatest(npmRegistry, name) {
    try {
        ...
        if (latest && latest !== packageDetails['dist-tags'].latest) {
            log(['update dist-tag'], `[${'latest dist'.cyan}] ${prettyName} ${latest}`);
            await execute(`npm dist-tag add ${prettyName}@${latest}`, { stdio: [0, 1, 2] });
        }
        else {
            log(['update dist-tag'], `[${'skipping update dist'.yellow}] ${prettyName}`);
        }
    }
    catch (error) {
