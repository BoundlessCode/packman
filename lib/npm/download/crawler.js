...
const { provider } = require('../NpmPackageProvider');

const { defaultRegistry: npmRegistry, maxRetries, requestTimeout } = provider;
...
async function _retrievePackageVersion({ name, version, outputPrefix = '', registry = npmRegistry, logger }) {
...
async function _getDependenciesFrom(dependenciesObject, outputPrefix, registry = npmRegistry, logger) {
