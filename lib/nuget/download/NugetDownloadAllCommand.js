    ...
    for (const page of pages.items) {
      ...
      pageResults.items.forEach(async (item) => {
        ...
        if (tracker.contains(packageInfo)) {
          itemLogger.info('skipping'.magenta, 'already tracked', packageFilePath);
        }
        else if (fs.existsSync(packageFilePath)) {
          if (track) {
            await tracker.track(packageInfo);
          }
          itemLogger.info('skipping'.magenta, 'file exists', packageFilePath);
        }
        else {
          try {
            itemLogger.info('starting download:'.cyan, packageFileName, packageFilePath);
            ...
            itemLogger.info('finished download:'.green, packageFileName, `in ${duration}`, packageFilePath);
          }
          catch (error) {
            itemLogger.info('failed to download'.red);
